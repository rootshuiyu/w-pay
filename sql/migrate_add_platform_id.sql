-- 迁移脚本：为 pay_channel 和 orders 表添加 platform_id 列
...
-- 添加 platform_id 列到 orders 表
ALTER TABLE orders ADD COLUMN IF NOT EXISTS platform_id BIGINT DEFAULT 0;

-- 添加索引
CREATE INDEX IF NOT EXISTS idx_orders_platform ON orders (platform_id);

-- 添加注释
COMMENT ON COLUMN orders.platform_id IS '代收平台';
